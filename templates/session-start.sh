...
# relevio.md holds the full text for when it needs the detail, and the
# /kickoff and /handoff commands carry their own step-by-step rituals (a slash
...
if [ -z "${CLAUDE_PROJECT_DIR:-}" ]; then
  emit "relevio ERROR: the session-start hook ran without CLAUDE_PROJECT_DIR set, so it cannot find relevio.md. Tell the user: relevio is installed but cannot confirm its own methodology file is there. It is likely registered with a hand-edited command in .claude/settings.json; re-running the installer restores the correct entry."
  exit 0
fi
METHODOLOGY="$CLAUDE_PROJECT_DIR/relevio.md"
if [ ! -r "$METHODOLOGY" ] || [ ! -s "$METHODOLOGY" ]; then
  emit "relevio ERROR: $METHODOLOGY is missing, unreadable or empty. The session cycle below still applies, but the full methodology it points to is NOT available, so /kickoff and /handoff will be working without their reference text. Tell the user rather than pretending everything is in place. Fix: restore relevio.md (it is committed with the project in team mode) or re-run the installer from the project root.
