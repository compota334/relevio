...
CORE="relevio: this project runs the relevio session cycle. It governs sessions ONLY (how they open, are paced against the context window, and close). How this project codes, verifies and handles errors is in CLAUDE.md, which relevio never touches. Full text in templates/relevio.md inside the relevio plugin: read it when you need the detail.
