...
#
# CLAUDE.md is yours and is left alone, with one exception: an install from
# v0.17 or earlier kept relevio's methodology inside it between markers, and
# that block is removed here too. Nothing outside the markers is touched.
#
...
if [ -f "$SETTINGS" ]; then
  ...
  jq -e . "$SETTINGS" >/dev/null 2>&1 || fail "$SETTINGS is not valid JSON. Clean it by hand: remove the
       PostToolUse entry referencing context-warn.sh and the SessionStart entry
       referencing session-start.sh."
  TMP="$(mktemp)"
  # Drop relevio's entries from both events, then prune whatever is left empty
  # so an uninstall does not leave hollow scaffolding behind.
  jq '
    def drop($ev; $needle):
      if .hooks[$ev] then
        .hooks[$ev] = [ .hooks[$ev][]
          | select(((.hooks // []) | map(.command // "") | any(contains($needle))) | not) ]
        | if .hooks[$ev] == [] then del(.hooks[$ev]) else . end
      else . end;
    drop("PostToolUse"; "context-warn.sh")
    | drop("SessionStart"; "session-start.sh")
  ...
  if [ "$(cat "$TMP")" = "{}" ]; then
    ...
    rm "$TMP"
    info "unchanged: $SETTINGS (no relevio hooks registered)"
  else
    mv "$TMP" "$SETTINGS"
    info "updated: $SETTINGS (relevio hooks unregistered, rest preserved)"
  fi
fi
rmdir .claude 2>/dev/null || true

# --- 3. Legacy CLAUDE.md block (installs from v0.17 and earlier) ------------
if [ -f CLAUDE.md ] && grep -qF "$MARK_START" CLAUDE.md; then
  ...
  # kill the script right when CLAUDE.md contains nothing but our old section.
  LEFT="$(grep -v '^[[:space:]]*$' CLAUDE.md.tmp | grep -vx '# Instructions for agents' | wc -l)" || true
  if [ "$LEFT" -eq 0 ]; then
    rm CLAUDE.md CLAUDE.md.tmp
    info "removed: CLAUDE.md (it only contained relevio's old section)"
  else
    mv CLAUDE.md.tmp CLAUDE.md
    info "updated: CLAUDE.md (relevio's old section removed, your content kept)"
  fi
else
  info "unchanged: CLAUDE.md (nothing of relevio's in it)"
fi
