---
description: Find an old session in the handoff library and get the command to reopen its conversation
argument-hint: <topic, date, or session-title fragment>
---
...
4. Answer with: the session name, the handoff file path, one line of what that
   session did, and how to reopen it. On Claude Code, give the LITERAL reopen
   command, copy-ready:

       claude --resume <session-id>

   (run from this project's root; in the Claude Code UI the user can instead
   just click the conversation with that name in the session list). On ZCode
   there is no `claude --resume`: point the user at the conversation in
   ZCode's own session list (a `sess_...` id in the `Resume:` line identifies
   it).
5. Warn whoever reopens it: a revisited session is for ASKING, not for working.
   ...
   session started with the kickoff command (`/relevio:kickoff`; plain
   `/kickoff` on ZCode).
...
HOST NOTE (ZCode): every relevio command is unprefixed on ZCode (`/kickoff`,
`/handoff`, `/revisit`, never `/relevio:...`), and there is no `/rename`
either: sessions are renamed and reopened from ZCode's own session list.
